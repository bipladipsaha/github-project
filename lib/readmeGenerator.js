 ...
 * Only includes sections that the user has actually filled in.
 */

import { computeFilledSections } from './sectionVisibility';
...
export function generateReadme(formData) {
  ...
  // Build README parts conditionally
  const parts = [];

  // Header (always shown if section is visible)
  if (sections.header) {
...
${badgesBlock}
  ...
  // Iterate over dynamic ordered sections
  (sections.orderedVisibleSections || []).forEach(id => {
    if (id === 'about') {
      parts.push(`
...
<picture><source media="(prefers-color-scheme: dark)" srcset="https://github-readme-activity-graph.vercel.app/graph?username=${githubUsername}&bg_color=00000000&color=ffffff&line=ffffff&point=ffffff&area_color=ffffff&area=true&hide_border=true&radius=0&custom_title=CONTRIBUTION%20TELEMETRY"/><img src="https://github-readme-activity-graph.vercel.app/graph?username=${githubUsername}&bg_color=00000000&color=000000&line=000000&point=000000&area_color=000000&area=true&hide_border=true&radius=0&custom_title=CONTRIBUTION%20TELEMETRY" width="97%" alt="GitHub contribution activity"/></picture>
      ...
      if (customData) {
        const titleSafe = customData.title.toLowerCase().replace(/\s+/g, '-');
        parts.push(`
<picture><source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/${githubUsername}/${githubUsername}/main/assets/dark/${id}-divider.svg"/><img src="https://raw.githubusercontent.com/${githubUsername}/${githubUsername}/main/assets/${id}-divider.svg" alt="${titleSafe} section"/></picture>
<picture><source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/${githubUsername}/${githubUsername}/main/assets/dark/${id}-content.svg"/><img src="https://raw.githubusercontent.com/${githubUsername}/${githubUsername}/main/assets/${id}-content.svg" alt="${titleSafe} content"/></picture>`);
      }
    }
  });
