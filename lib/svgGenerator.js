 ...
 */
function buildReplacements(formData) {
  const {
    fullName = 'Your Name',
  ...
  for (const filename of SVG_FILES) {
    for (const isDark of [false, true]) {
      ...
      try {
        ...
        // Trim projects.svg to 3 rows if user has ≤ 3 projects
        if (filename === 'projects.svg' && (formData.projects || []).length <= 3) {
          content = content.replace(/<g class="row r4"[\s\S]*?(?=<\/g>\s*<\/svg>)/, '');
          content = content.replace('viewBox="0 0 1000 1690"', 'viewBox="0 0 1000 1310"');
        }
